...
(function () {
    ...
    image.addEventListener('mousedown', (/** @type {MouseEvent} */ e) => {
        if (!image || !hasLoadedImage) {
            return;
        }
    ...
    image.addEventListener('click', (/** @type {MouseEvent} */ e) => {
        if (!image || !hasLoadedImage) {
            return;
        }
    ...
    image.addEventListener('wheel', (/** @type {WheelEvent} */ e) => {
    ...
    // Add a toggle button to allow user to quickly swap between original and generated ASCII version
    let toggleBtn = document.createElement("button");
    toggleBtn.innerText = "Toggle ASCII / Original";
    toggleBtn.onclick = (element, ev) => {
        toggleTabContent('image-preview-container', 'ascii-preview-container');
    };

    document.getElementById('button-container').appendChild(toggleBtn);

    // Add a copy to clipboard button.
    let copyBtnText = "Copy to Clipboard";
    let copyBtn = document.createElement("button");
    copyBtn.innerText = copyBtnText;

    // Give status message directly on the button and reset after 3s
    copyBtn.onclick = (element, ev) => {
        navigator.clipboard.writeText(document.getElementById("converted-ascii").innerText).then(function() {
            copyBtn.innerText = "Copied!";
          }, function(err) {
            copyBtn.innerText = "Couldn't Copy!";
          }).finally(() => {
              setTimeout(() => { copyBtn.innerText = copyBtnText; }, 3000);
            }
          );
    };

    // Logic for toggling original/ascii div's
    document.getElementById('button-container').appendChild(copyBtn);
    
	function toggleTabContent(contentFirst, contentSecond) {
        let firstElement = document.getElementById(contentFirst);
        let secondElement = document.getElementById(contentSecond);

        if (firstElement.classList.contains('hide')) {
            secondElement = [firstElement, firstElement = secondElement][0]; // Swap
        }
